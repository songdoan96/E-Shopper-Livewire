...
class SearchComponent extends Component
{
    ...
    public function render()
    {
        if ($this->sorting == "date") {
            $products = Product::where('name', 'like', '%' . $this->search . '%')->where('status', '1')->orderBy('created_at', 'desc')->paginate($this->pagesize);
        } else if ($this->sorting == "price_asc") {
            $products = Product::where('name', 'like', '%' . $this->search . '%')->where('status', '1')->orderBy('price', 'asc')->paginate($this->pagesize);
        } else if ($this->sorting == "price_desc") {
            $products = Product::where('name', 'like', '%' . $this->search . '%')->where('status', '1')->orderBy('price', 'desc')->paginate($this->pagesize);
        } else {
